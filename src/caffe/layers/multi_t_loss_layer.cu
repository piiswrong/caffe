...
void MultiTLossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  Dtype alpha_exp =  Dtype((alpha+K_)/2.0);
  Dtype alpha_weight = Dtype((alpha+K_)/alpha);
  ...
  for (int i = 0; i < bottom[0]->num(); i++) {
    ...
    for (int j = 0; j < N_; j++) {
      ...
      norm += cpu_inv_sigma_prod[j]*std::pow(weight, alpha_exp);
...
void MultiTLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    caffe_gpu_mul(mask_.count(), (*bottom)[0]->gpu_diff(), (*bottom)[0]->gpu_data(), (*bottom)[0]->mutable_gpu_diff());
    caffe_gpu_gemm(CblasNoTrans, CblasTrans, mask_.num(), K_, N_, -weight,
