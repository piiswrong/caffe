...
void TLossLayer<Dtype>::LayerSetUp(
      const vector<Blob<Dtype>*>& bottom, vector<Blob<Dtype>*>* top) {
  Layer<Dtype>::LayerSetUp(bottom, top);
  N_ = this->layer_param_.t_loss_param().num_center();
  lambda_ = this->layer_param_.t_loss_param().lambda();
  ...
  if (this->blobs_.size() > 0) {
    ...
    Dtype *cpu_sigma2 = this->blobs_[1]->mutable_cpu_data();
    Dtype *cpu_pi = this->blobs_[2]->mutable_cpu_data();
    for (int i = 0; i < N_; i++) {
      cpu_sigma2[i] = Dtype(1);
...
template <typename Dtype>
void TLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    vector<Blob<Dtype>*>* top) {
  LOG(ERROR) << "Forward_cpu not implemented.";
}

template <typename Dtype>
void TLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
