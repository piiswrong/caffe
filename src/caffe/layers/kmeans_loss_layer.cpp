...
template <typename Dtype>
void KmeansLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    vector<Blob<Dtype>*>* top) {
  LOG(ERROR) << "Forward_cpu not implemented.";
}

template <typename Dtype>
void KmeansLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, vector<Blob<Dtype>*>* bottom) {
  LOG(ERROR) << "Backward_cpu not implemented.";
}

#ifdef CPU_ONLY
STUB_GPU(KmeansLossLayer);
#endif
