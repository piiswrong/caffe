 ...
 protected:
  MultiTLossLayerTest()
      : blob_bottom_data_(new Blob<Dtype>(dim, 1, 1, dim)),
        blob_bottom_label_(new Blob<Dtype>(dim, 1, 1, dim)),
    ...
    for (int i = 0; i < dim; i++) {
      Dtype norm = Dtype(0);
      for (int j = 0; j < dim; j++) {
        norm += blob_bottom_label_->mutable_cpu_data()[i*dim+j];
      }
      for (int j = 0; j < dim; j++) {
        blob_bottom_label_->mutable_cpu_data()[i*dim+j] /= norm;
      }
    }
  ...
  void TestForward() {
    ...
    layer_param.mutable_multi_t_loss_param()->set_num_center(dim);
    ...
    layer_param2.mutable_multi_t_loss_param()->set_num_center(dim);
    ...
    int m = dim, n = layer_param.multi_t_loss_param().num_center(), p = dim;
